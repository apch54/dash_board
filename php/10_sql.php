...
function choix_bd()
{
   //return '_dv';
    ...
    if (isset($v_ou_p) and ($v_ou_p == 'o')) {
        return '_o';
    }
    if (isset($v_ou_p) and ($v_ou_p == 'at')) {
        return '_at';
    }
}
...
function inner_join($dv)
{
    return "
     INNER JOIN (
        ...
        UNION
            SELECT jeux, 'AUTOPROMO' ,'Autopromo'
...
function modifs_qry($date_1)
{
    ...
    switch ($dv) {
        ...
        case '_o'   : $dv_dp = 'O';             break;
        case '_at'  : $dv_dp = 'AT';            break;
